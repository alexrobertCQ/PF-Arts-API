...
const artworksPaging = async (pag=1,century,order,created) => {
  ...
  const validate=(pag,century,order,created)=>{
    ...
    if (pag && isNaN(pag)) throw Error('Invalid pagin range');
    if (century && isNaN(century) || century<1 || century>21) throw Error('Invalid century range');
    if (order && order!='ASC'&&order!='DESC') throw Error('Invalid order')
    if (created && created!='true' && created!='false') throw Error('invalid creation range')
  }
